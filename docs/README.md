...
- [`format/CATALOG_SCHEMA_V4.md`](format/CATALOG_SCHEMA_V4.md)
- [`format/CATALOG_SCHEMA_V5.md`](format/CATALOG_SCHEMA_V5.md)
...
- ADR-0001 through ADR-0033 record the foundational ownership, chunking, envelope, catalog, decoy, FFI, local-first, container-freeze, label-registry, canonical-tuple, descriptor-authentication, import-journal, constant-allocation, sync-causality, C-ABI, supported-device, backup-framing, record-layout, parser-limit, tie-break, chain-hash, checkpoint, revocation, envelope-AAD, password-KDF, deletion-ordering, query-surface, recovery-encoding, vault-discovery, CI-ownership, provisioning, and server-model decisions. Number 0015 was not used.
...
| `membership_generation` | [`sync/DEVICE_IDENTITY.md`](sync/DEVICE_IDENTITY.md) §4 | accepted device-membership states of one vault | yes, and on the wire | yes |
...
"Handle generation" in prose is the session counter of the last row, and `session_generation` in [`format/OBJECT_CONTAINER_V1.md`](format/OBJECT_CONTAINER_V1.md) §12 is the same value. The "global/materialized state generation" of [`sync/ROLLBACK_PROTECTION.md`](sync/ROLLBACK_PROTECTION.md) §2 is not defined in v1 and takes a row here in the change that defines it. The generation digit of a file magic, [`format/CANONICAL_ENCODING_V1.md`](format/CANONICAL_ENCODING_V1.md) §15.1, is not a counter; it is the eighth byte of the magic. The `membership_generation` of a collection grant, [`sync/COLLECTION_GRANTS.md`](sync/COLLECTION_GRANTS.md) §2, of a `RevokeDeviceRecordV1`, [`sync/DEVICE_IDENTITY.md`](sync/DEVICE_IDENTITY.md) §9, and of a `CheckpointV1`, [`sync/ROLLBACK_PROTECTION.md`](sync/ROLLBACK_PROTECTION.md) §6, are the one vault counter of the row above, which is what lets a freshness floor taken from an enrollment record reject a stale grant.
