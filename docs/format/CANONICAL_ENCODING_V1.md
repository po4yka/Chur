...
No allocated tag is a byte prefix of another, as §7 requires; the eighteen above were checked pairwise. `CHUR\x00CATALOG\x00HEADER-COMMITMENT\x00V1` differs from every other allocated tag at byte 5, `C` against `K`, `S`, `O`, `V`, `B`, or `I`, and no other tag begins with `CHUR\x00C`. The two sync tags differ at the first byte after `CHUR\x00SYNC\x00`, and `CHUR\x00IDENTITY\x00FINGERPRINT\x00V1` differs from every other allocated tag at byte 5. The four slot tags share `CHUR\x00SLOT\x00` and differ at byte 11, `P`, `R`, `A`, and `A`, with the two `A` tags differing at byte 12, `N` against `P`. `CHUR\x00SLOT\x00` and `CHUR\x00SYNC\x00` differ at byte 6, `L` against `Y`.
