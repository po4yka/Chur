...
Local crypto-erasure requires removal of every locally accessible envelope for an object key, including current catalog state, WAL/journal copies, backup snapshots, and queued sync operations.
...
### Phase 0 — specifications and core prototypes

- threat model and invariants;
- key-slot specification;
- object container v1 specification;
- object-key envelope specification;
- canonical encoding ADR;
- SQLCipher catalog prototype;
- FFI handle/session prototype;
- `chur-cli` and golden vectors;
- Android/iOS key-protector prototypes;
- corruption and fuzz harnesses.
...
- encrypted catalog;
- photo import and immutable original containers;
- encrypted metadata and thumbnails;
- grid, timeline, viewer, export;
- immediate/timed/background lock;
- task/app-switcher privacy;
- integrity inspection and crash recovery.

Excluded: cloud accounts, sync, sharing, decoy vault, local AI.

### Phase 2 — video, audio, and decoy

- Media3 and AVFoundation range readers;
- encrypted previews, poster frames, and waveforms;
- large-file benchmarks and tuning;
- independent decoy vault;
- stricter platform privacy modes;
- portable encrypted backup;
- complete fault-injection matrix.

### Phase 3 — backup and synchronization

- opaque server object storage;
- ciphertext-only background transfer;
- encrypted canonical catalog operations;
- device identities and signed logs;
...
- Ed25519 identity and verification;
- permissions, membership changes, and collection epochs;
- revocation semantics;
- multi-recipient compatibility vectors;
...
- shared family/team vaults after the security model matures.
