...
| FFI ABI | 1.1, capabilities `0x000000000000000E` |
...
| Suite | Count | Command |
| --- | --- | --- |
| Rust | 366 tests, 0 failed | `cd rust && cargo test --workspace` |
| Kotlin | 186 tests, 0 failed | `./gradlew build -x lint` |
...
| password and recovery slots | `vault::tests::a_created_vault_unlocks_with_its_password`, `the_recovery_slot_offered_during_creation_unlocks_the_vault`, `a_recovery_slot_added_after_activation_unlocks_the_vault`; `chur-crypto` `tests/argon2_cost.rs` proves a password unlock performs exactly two derivations whether it succeeds or fails |
| device slot | `vault::tests::an_apple_keychain_slot_unlocks_the_vault`. There is no Android equivalent, because there is no Android unlock factor; see §6 |
...
| metadata, thumbnails, previews | `tests/pipeline.rs::a_derived_asset_round_trips_and_sets_the_thumbnail_flag`, `a_derivative_above_its_long_edge_is_refused` |
...
[`../interop/FFI_CONTRACT.md`](../interop/FFI_CONTRACT.md) §6.2 froze a control plane that could not create a vault, mark a favourite, delete an object, or read a thumbnail. §6.5 adds the product surface those flows need, and the ABI minor version moved to 1.1 because a host built against 1.0 still links.

| Fact | Value | Test |
| --- | --- | --- |
| C functions declared in `chur.h` | 47 | `chur-ffi` `tests/header.rs::every_declared_function_is_exported` |
| JNI functions in `chur-jni` | 47 | `chur-jni` `tests/surface.rs`, which compares both directions |
...
- **the Android Keystore slot does not unlock.** `chur-format` carries `AndroidKeystoreSlotBody`, its AAD, and wrap suite `0x0002`, and `:shared:core-platform-keys` wraps and unwraps through the Keystore. No unlock factor is allocated behind `CHUR_FACTOR_*` and no export accepts one, so on Android a password or the recovery phrase is the only way in;
...
- **the iOS grid draws no thumbnails.** The derivatives exist and Rust returns them; no platform decode is bound on that side yet;
...
- **selection has no batch actions.** The library screen selects and the actions apply to one object at a time;
- **the panic transition has no gesture.** It runs, and [`../product/DISCREET_MODE.md`](../product/DISCREET_MODE.md) reserves the choice of gesture to itself and has not made it;
