...
Individual statements are classified as **Decision**, **Invariant**, **Proposal**, **Deferred**, or **Non-goal**, defined once in [`README.md`](README.md#statement-classification). An **Invariant** here is a property every iOS implementation preserves. The status of this document as a whole is in its header and uses the document-status vocabulary of the same file.
