...
The versioned, length-bounded command channel lists objects, albums, and tags; manages album trees and manual order; and applies album membership, tags, and favorites individually or in bounded batches through `VaultRepository`. A remote import exposes one host file through a private, non-exported provider backed by a seekable proxy descriptor. The provider requests bounded byte ranges over the paired channel, so no plaintext source file is staged on Android. The ordinary Android codec and Rust import transaction remain responsible for probing, derivatives, encryption, and commit. A failed or interrupted request is reported as a failure; a committed original is never rolled back merely because a later derivative failed. An export uses a leased reader to stream the authenticated original sequentially and returns a SHA-256 digest when complete; the desktop verifies that digest before publishing its temporary file.

This session authorizes the currently open identity only. It does not unlock a vault, keep one unlocked in the background, access the sibling identity, or expose the catalog as a filesystem. The dialog shows whether a computer is connected; the session also expires after ten minutes without a command. Desktop output can contain private names and IDs, so the desktop user is responsible for where command output is stored.
