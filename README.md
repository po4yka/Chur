...
### Phase 0 — protocol and repository foundation

- finalize threat model;
- specify key slots and password encoding;
- specify object container v1;
- specify object-key envelopes;
- prototype the Rust-owned catalog;
- establish FFI ownership and session invalidation;
- create `chur-cli` and golden vectors;
- configure Android/iOS cross-compilation and CI;
- add security-focused linting, dependency review, and fuzz harnesses.

### Phase 1 — local photo vault

- functional Notes public shell;
- one recoverable private vault;
- password and platform key slots;
- Android Photo Picker and iOS PhotosPicker import;
- encrypted photo originals, thumbnails, and metadata;
- timeline/grid/viewer;
- export with explicit plaintext cleanup;
- immediate and timed lock;
- app-switcher privacy handling;
- integrity inspection and interrupted-import recovery.

Not included: accounts, cloud sync, sharing, decoy vault, local AI.

### Phase 2 — video, audio, and decoy

- Media3 and AVFoundation random-access readers;
- encrypted previews, poster frames, and waveforms;
- large-file performance and battery tuning;
- independent decoy vault;
- stricter capture/background policies;
- full corruption and recovery test matrix;
- encrypted interoperable export/backup.

### Phase 3 — encrypted backup and sync

- opaque object upload/download;
- encrypted canonical catalog operations;
- device identities;
- signed per-device operation logs;
- rollback/replay detection;
- conflict and tombstone semantics;
- collection epochs and recovery across devices;
- ciphertext-only background transfer.

### Phase 4 — sharing

- X25519 HPKE collection grants;
- Ed25519 sender/device identity;
- recipient verification;
- permissions and revocation semantics;
- key rotation after membership changes;
- multi-device and multi-recipient interoperability tests;
- separate protocol audit.

### Later exploration

- encrypted documents;
- local OCR and semantic search;
- locally generated embeddings stored in the vault;
- private multimodal search over photos, video, and audio;
- optional hybrid post-quantum recipient formats;
- shared family/team vaults after the security model is mature.
