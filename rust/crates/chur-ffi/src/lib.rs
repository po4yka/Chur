...
///
/// It is 1 rather than 0 because `docs/interop/FFI_CONTRACT.md` §6.5 adds the
/// exports Phase 1's product scope requires, and §6.2 makes an addition a minor
/// bump. Nothing in the §6.2 list changed, so a host built against 1.0 still
/// works: an export it does not call costs it nothing.
...
mod tests {
    ...
    fn the_handshake_answers_every_documented_fact() {
        ...
        assert_eq!(chur_abi_version_minor(), 1);
