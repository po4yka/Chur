...
use chur_format::constants::{CATALOG_FORMAT_VERSION_V6, MediaClass, ObjectState};
...
fn a_vault_this_build_cannot_read_fails_closed() {
    for point in MIGRATION_POINTS {
        ...
        let store_id = session.object_store_id();
        let key = CatalogKey::derive(session.root_secret().expect("root"), &vault_id)
            .expect("catalog key");
        drop(session);
                ...
                db.transaction(|transaction| {
                    transaction
                        .execute(
                            ...
                            [i64::from(CATALOG_FORMAT_VERSION_V6) + 1],
                        )
                        .map(|_| ())
                        .map_err(|_| {
                            chur_core::Error::new(
