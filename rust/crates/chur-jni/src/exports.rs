...
//! returns the export's status unchanged. Every body is wrapped in
//! [`crate::convert::contain`], which is §11 of `docs/interop/FFI_CONTRACT.md`
//! applied to this artifact too: a panic in the argument conversions or the
//! result writes — the parts that run outside `chur-ffi`'s own guard —
//! returns the §11 default rather than unwinding across `extern "system"`.
...
pub extern "system" fn Java_dev_po4yka_chur_ffi_ChurJni_statusIsKnown(
    ...
    crate::convert::contain(jboolean::from(false), || {
...
pub extern "system" fn Java_dev_po4yka_chur_ffi_ChurJni_vaultPresent<'local>(
    ...
    crate::convert::contain_env(INTERNAL_FAILURE, env, |env| {
        ...
        let status =
            unsafe { chur_ffi::product::chur_vault_present(handle_of(runtime), &mut present) };
        if status != 0 {
            return status;
        }
...
/// Returns the default collection and active recipient devices.
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_po4yka_chur_ffi_ChurJni_sharingOverview<'local>(
    env: EnvUnowned<'local>,
    _class: JClass<'local>,
    session: jlong,
    destination: JByteBuffer<'local>,
    ...
    crate::convert::contain_env(INTERNAL_FAILURE, env, |env| {
        ...
        let mut written = 0usize;
        // SAFETY: `written` is live and the buffer is direct.
        let status = unsafe {
            chur_ffi::sharing::chur_sharing_overview(
                handle_of(session),
                address,
                capacity,
                &mut written,
...
/// Authenticates an initial recipient enrollment before display.
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_po4yka_chur_ffi_ChurJni_sharingInspectEnrollment<'local>(
    env: EnvUnowned<'local>,
    _class: JClass<'local>,
    ...
    crate::convert::contain_env(INTERNAL_FAILURE, env, |env| {
        let Some(enrollment) = byte_array(env, &enrollment) else {
            return INVALID_INPUT;
        };
        let Ok(enrollment_length) = u32::try_from(enrollment.len()) else {
            return INVALID_INPUT;
        };
        let Some((address, capacity)) = direct_buffer(env, &destination) else {
            return INVALID_INPUT;
        ...
        let mut written = 0usize;
        // SAFETY: the vector, `written`, and direct buffer stay live for the call.
        let status = unsafe {
            chur_ffi::sharing::chur_sharing_inspect_enrollment(
                enrollment.as_ptr(),
                enrollment_length,
                address,
                capacity,
...
pub extern "system" fn Java_dev_po4yka_chur_ffi_ChurJni_operationPoll<'local>(
    ...
    crate::convert::contain_env(INTERNAL_FAILURE, env, |env| {
        ...
        let status =
            unsafe { chur_ffi::api::chur_operation_poll(handle_of(operation), &mut progress) };
        if status != 0 {
            return status;
        }
...
pub extern "system" fn Java_dev_po4yka_chur_ffi_ChurJni_objectReaderOpen<'local>(
    ...
    crate::convert::contain_env(INTERNAL_FAILURE, env, |env| {
        ...
        // SAFETY: `reference` and `handle` are live locals.
        let status = unsafe {
            chur_ffi::api::chur_object_reader_open(
                handle_of(session),
                &reference,
                kind,
                &mut handle,
...
pub extern "system" fn Java_dev_po4yka_chur_ffi_ChurJni_objectReaderSize<'local>(
    ...
    crate::convert::contain_env(INTERNAL_FAILURE, env, |env| {
        ...
        let status =
            unsafe { chur_ffi::api::chur_object_reader_size(handle_of(reader), &mut size) };
        if status != 0 {
            return status;
        }
...
pub extern "system" fn Java_dev_po4yka_chur_ffi_ChurJni_objectReaderVerifyComplete<'local>(
    ...
    crate::convert::contain_env(INTERNAL_FAILURE, env, |env| {
        ...
        let status = unsafe {
            chur_ffi::api::chur_object_reader_verify_complete(handle_of(reader), &mut state)
        };
        if status != 0 {
            return status;
        }
